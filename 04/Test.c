...
//从1~x个结点选择parent节点为0,权重最小的两个节点 
{
     ...
     {
         if(!ht[i].parent) //父节点为空(节点的parent=0) 
         {
             if(ht1==NULL) //节点指针1为空 
             {
                 ht1=ht+i; //指向第i个节点 
             ...
             {
                 ht2=ht+i; //指向第i个节点 
                 if(ht1->weight>ht2->weight) //比较两个节点的权重，使ht1指向的节点权重小 
                 {
                     t=ht2;
                     ht2=ht1;
                     ht1=t;
             ...
             if(ht1 && ht2) //若ht1、ht2两个指针都有效 
             {
                 if(ht[i].weight<=ht1->weight) //第i个节点权重小于ht1指向的节点 
                 {
                     ...
                     ht1=ht+i; //ht1指向第i个节点 
                 }else if(ht[i].weight<ht2->weight){ //若第i个节点权重小于ht2指向的节点 
                     ht2=ht+i; //ht2指向第i个节点 
                 }
             }
         }
...
void CreateTree(HuffmanTree *ht,int n,int *w)
{
    ...
    {
        SelectNode(ht,i-1,&bt1,&bt2); //从1~i-1个节点选择parent节点为0,权重最小的两个节点 
        ht[bt1].parent=i;
        ht[bt2].parent=i;
        ht[i].left=bt1;
        ht[i].right=bt2;
