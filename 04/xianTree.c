...
ThreadBinTree *Previous=NULL;     //前驱节点指针 

ThreadBinTree *BinTreeInit(ThreadBinTree *node) //初始化二叉树根节点 
{
...
ThreadBinTree *BinTreeLeft(ThreadBinTree *bt) //返回左子节点 
{
    if(bt)
        return bt->left;
    else
...
ThreadBinTree *BinTreeRight(ThreadBinTree *bt) //返回右子节点 
{
    if(bt)
        return bt->right;
    else
...
ThreadBinTree *BinTreeFind(ThreadBinTree *bt,DATA data) //在二叉树中查找值为data的节点 
{
    ThreadBinTree *p;
    if(bt==NULL)
        return NULL;
...
ThreadBinTree *BinTreeNext_LDR(ThreadBinTree *bt) //求指定节点的后继 
{
    ThreadBinTree *nextnode;
    if(!bt) return NULL; //若当前节点为空，则返回空 
    if(bt->rflag==Thread) //若当前节点的右子树为空
        return bt->right; //返回右线索所指的中序后继
    else{
        nextnode=bt->right; //从当前节点的右子树开始查找
        while(nextnode->lflag==SubTree) //循环处理所有左子树不为空的节点
            nextnode=nextnode->left;
        return nextnode; //返回左下方的节点
    }
}
ThreadBinTree *BinTreePrevious_LDR(ThreadBinTree *bt) //求指定节点的前驱 
{
    ThreadBinTree *prenode;
    if(!bt) return NULL; //若当前节点为空，则返回空 
    if(bt->lflag==Thread) //若当前节点的左子树为空
        return bt->left; //返回左线索所指的中序后继
    else{
        prenode=bt->left; //从当前节点的左子树开始查找
        while(prenode->rflag==SubTree) //循环处理所有右子树不为空的节点
            prenode=prenode->left;
        return prenode; //返回左下方的节点
    }
}
...
void ThreadBinTree_LDR(ThreadBinTree *bt,void (*oper)(ThreadBinTree *p)) //遍历中序线索二叉树
{
    if(bt) //二叉树不为空
    {
        while(bt->lflag==SubTree)//有左子树 
            bt=bt->left; //从根往下找最左下节点，即中序序列的开始节点
        do{
           oper(bt); //处理节点 
           bt=BinTreeNext_LDR(bt);//找中序后继节点
        }while(bt);
    }
}
